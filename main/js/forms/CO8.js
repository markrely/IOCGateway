...
var additionalPrincipalPaymentAmountFlag = 0;
 $('#additionalPaymentAmt').on('input',function(e){
     if($(this).data("lastval")!= $(this).val() && $(this).val() !== ""){
     $(this).data("lastval",$(this).val());
        //change action
        //alert("Please add comments.");
        additionalPrincipalPaymentAmountFlag = 1;
     };
 });
...
//END MARKS JQUERY
  ...
  $("#paidOnDate").blur(function () {
   ...
   $("#dateTrusteeReceived").blur(function () {
      if($("#dateTrusteeReceived").val() !== ""){
         ValidateDate($('#dateTrusteeReceived').val(), "dateTrusteeReceived","Date Trustee Received");
      }
     if(new Date($("#dateTrusteeReceived").val()) > new Date() && $("#dateTrusteeReceived").val() !== ""){
      $("#messageHolder").attr("class","alert alert-danger").html("You can not Enter Future date.");
      $("#messageHolderBtm").attr("class","alert alert-danger").html("You can not Enter Future date.");
      $(".dateTrusteeReceived").attr("class","col-sm-9 dateTrusteeReceived has-error");
      $(":submit").attr("disabled", true);
      }
      else{
        $("#messageHolder").attr("class","hide").html("");
        $("#messageHolderBtm").attr("class","hide").html("");
        $(".dateTrusteeReceived").attr("class","col-sm-9 dateTrusteeReceived");
        $(":submit").attr("disabled", false);
      }
  });
  ...
  $(":submit").click(function(event){
    if(new Date($("#dueOndate").val()) > new Date()){
      ...
      else if($("#hdnIsCalledDate").val() == 1 && $("#Comment").val() == "" ){
             $("#messageHolder").attr("class","alert alert-danger").html("Please Enter some comments.");
             $("#messageHolderBtm").attr("class","alert alert-danger").html("Please Enter some comments.");
             return false;
        }
        else if(additionalPrincipalPaymentAmountFlag == 1 &&  $("#Comment").val() == "" ){
             $("#messageHolder").attr("class","alert alert-danger").html("Please Enter some comments for the Additional Principal Payment Amount that you have added.");
             $("#messageHolderBtm").attr("class","alert alert-danger").html("Please Enter some comments for the Additional Principal Payment Amount that you have added.");
             return false;
        }
        else{
          $("#messageHolder").attr("class","hide").html("");
