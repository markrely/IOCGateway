! function(e) {
    ...
    var r = function(t, n) {
        ...
        this.format = s.parseFormat(n.format || this.element.data("date-format") || "mm/dd/yyyy");
        this.isInline = false;
    ...
    r.prototype = {
        ...
        place: function() {
            ...
            var t = parseInt(this.element.parents().filter(function() {
                ...
                r = this.component ? this.component.outerHeight(true) : this.element.outerHeight(true);
            this.picker.css({
                top: n.top + r,
                left: n.left,
                zIndex: t
        ...
        fill: function() {
            var n = new Date(this.viewDate),
                ...
                c = this.endDate !== Infinity ? this.endDate.getUTCMonth() : Infinity,
                h = this.date && this.date.valueOf(),
                p = new Date;
            this.picker.find(".datepicker-days thead th:eq(1)").text(i[this.language].months[o] + " " + r);
            this.picker.find("tfoot th.today").text(i[this.language].today).toggle(this.todayBtn !== false);
            this.updateNavArrows();
            this.fillMonths();
            var d = t(r, o - 1, 28, 0, 0, 0, 0),
                v = s.getDaysInMonth(d.getUTCFullYear(), d.getUTCMonth());
            d.setUTCDate(v);
            d.setUTCDate(v - (d.getUTCDay() - this.weekStart + 7) % 7);
            var m = new Date(d);
            ...
            while (d.valueOf() < m) {
                ...
                y = "";
                if (d.getUTCFullYear() < r || d.getUTCFullYear() == r && d.getUTCMonth() < o) {
                    y += " old"
                } else if (d.getUTCFullYear() > r || d.getUTCFullYear() == r && d.getUTCMonth() > o) {
                    y += " new"
                }
                ...
                if (d.valueOf() < this.startDate || d.valueOf() > this.endDate || e.inArray(d.getUTCDay(), this.daysOfWeekDisabled) !== -1) {
                    y += " disabled"
                }
                g.push('<td class="day' + y + '">' + d.getUTCDate() + "</td>");
            ...
            var w = this.picker.find(".datepicker-months").find("th:eq(1)").text(r).end().find("span").removeClass("active");
            if (b && b == r) {
                w.eq(this.date.getUTCMonth()).addClass("active")
            }
            if (r < u || r > f) {
                w.addClass("disabled")
            }
            if (r == u) {
                w.slice(0, a).addClass("disabled")
            }
            if (r == f) {
                w.slice(c + 1).addClass("disabled")
            }
            g = "";
            r = parseInt(r / 10, 10) * 10;
            var E = this.picker.find(".datepicker-years").find("th:eq(1)").text(r + "-" + (r + 9)).end().find("td");
            r -= 1;
            for (var S = -1; S < 11; S++) {
                g += '<span class="year' + (S == -1 || S == 10 ? " old" : "") + (b == r ? " active" : "") + (r < u || r > f ? " disabled" : "") + '">' + r + "</span>";
                r += 1
            }
            E.html(g)
        },
